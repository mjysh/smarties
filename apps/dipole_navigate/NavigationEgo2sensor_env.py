...
class DipoleSingleEnv(gym.Env):
    ...
    def __init__(self, mode = 'CFD', dt = 0.1, alpha = 3, r = 3,mu = 0.8, flexibility = 0.5, train_offset=0.1,cfdpath='./',cfd_dumpinterval=10,cfd_framerate = 1000,cfd_inittime = 160):
        ...
        if (mode == 'CFD'):
            ...
            with np.load('CFDdata.npz') as data:
                self.Uf = data['U']
                self.Vf = data['V']
                self.Of = data['Omega']
                self.xf = data['X']
...
#        print(Style.RESET_ALL)
    ...
    def _get_obs(self):                 # get the orientation (reltative to the targeted direction) and the shape
        ...
        reluL = uVKL*np.cos(self.pos[-1])+ vVKL*np.sin(self.pos[-1])
        relvL = -uVKL*np.sin(self.pos[-1])+ vVKL*np.cos(self.pos[-1])
        reluR = uVKR*np.cos(self.pos[-1])+ vVKR*np.sin(self.pos[-1])
        relvR = -uVKR*np.sin(self.pos[-1])+ vVKR*np.cos(self.pos[-1])
        disToTarget = np.sqrt((self.pos[0]-self.target[0])**2+(self.pos[1]-self.target[1])**2)
        return np.array([theta,disToTarget,reluL,relvL,reluR,relvR])
