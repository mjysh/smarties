...
class DipoleSingleEnv(gym.Env):
    ...
    def __init__(self, mode = 'CFD', dt = 0.1, alpha = 3, r = 3,mu = 0.8, flexibility = 0.5, train_offset=0.1,cfdpath='/home/yusheng/newCFD/',cfd_dumpinterval=10,cfd_framerate = 1000,cfd_inittime = 150):
        # size and parameters
        self.speed = mu
        self.bl = 1
        self.bw = 0.2
        ...
        high = np.array([np.finfo(np.float32).max,np.finfo(np.float32).max,np.finfo(np.float32).max,np.finfo(np.float32).max, np.finfo(np.float32).max])
        low = np.array([-np.finfo(np.float32).max,-np.finfo(np.float32).max,-np.finfo(np.float32).max,-np.finfo(np.float32).max,-np.finfo(np.float32).max])
...
#        print(Style.RESET_ALL)
    ...
    def _get_obs(self):                 # get the orientation (reltative to the targeted direction) and the shape
        ...
        return np.array([self.target[0] - self.pos[0],self.target[1] - self.pos[1],
                         uVK,vVK,self.pos[-1]])
