...
class DipoleSingleEnv(gym.Env):
    ...
    def __VKflow(self, pos,t):
        ...
        # if t>29:
        #     print('t',t)
        uVK,vVK,oVK = cf.interp_vel_from_matrix(self.Uf,self.Vf,self.xf,self.yf,self.Of,posX=posx,posY=posy,time=t,frame_rate = self.cfd_framerate,read_interval = 5)
...
#        print(Style.RESET_ALL)
    ...
    def _get_obs(self):                 # get the orientation (reltative to the targeted direction) and the shape
        ...
        relx = (self.target[0] - self.pos[0])*np.cos(self.pos[-1]) 
        + (self.target[1] - self.pos[1])*np.sin(self.pos[-1])
        rely = -(self.target[0] - self.pos[0])*np.sin(self.pos[-1])
        + (self.target[1] - self.pos[1])*np.cos(self.pos[-1])
